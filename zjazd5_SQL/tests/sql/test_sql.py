...
class TestSql:
    ...
    def test_find_survivors(self, load_titanic_data):
        ...
        # df = run_query("SELECT passengers.Name AS Name FROM passengers JOIN survivors ON passengers.PassengerID = "
        #                "survivors.PassengerID WHERE survivors.Survived=1")

        df = run_query("SELECT Name FROM passengers p JOIN survivors s ON p.PassengerID = s.PassengerID "
                       "WHERE s.Survived=1")
    ...
    def test_count_by_embarked(self, load_titanic_data):
        ...
        df = run_query("SELECT Embarked AS embarked, COUNT(*) AS number_of_passengers FROM passengers p JOIN tickets t "
                       "ON p.PassengerID == t.PassengerID WHERE Embarked IS NOT NULL GROUP BY Embarked")
    ...
    def test_find_max_min_age_by_sex_and_survived(self, load_titanic_data):
        ...
        """
        df = run_query("SELECT Sex, Survived, MAX(Age) as Oldest, MIN(Age) as Youngest FROM passengers p "
                       "JOIN survivors s ON p.PassengerID = s.PassengerID WHERE Age IS NOT NULL GROUP BY Sex, Survived")
