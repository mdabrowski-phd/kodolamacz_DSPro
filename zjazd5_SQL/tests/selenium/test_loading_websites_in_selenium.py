...
def test_load_table_header_using_selenium(driver):
    ...
    headers = driver.find_elements(by=by.By.XPATH, value='//table/thead/tr/th')
...
def test_load_table_as_dataframe_using_selenium(driver):
    """Wczytaj stronę, której adres znajduje się w zmiennej URL przy użyciu biblioteki selenium.
    Wczytaj tabelę znajdujacą się na stronie jako DataFrame (tej samej, której używaliśmy w testach BeautifulSoup)
    """
    driver.get(URL)

    table = driver.find_element(by=by.By.XPATH, value='//table').get_attribute('outerHTML')
...
def test_follow_links_in_selenium(driver):
    ...
    url = driver.find_element(by=by.By.LINK_TEXT, value='page1')
    print(url.get_attribute('href'))

    # pierwszy sposób (uzyskanie linku i przejście na odpowiednią stronę)
    # driver.get(url.get_attribute('href'))

    # drugi sposób (kliknięcie elementu zawierającego link do podstrony
    url.click()

    time.sleep(1)
    title = driver.title  # page1
    print(title)
